...
# Magenta parking-lot limitations (tune on the real competition lighting)
...
initial_yaw = 0
yaw = 0
imu_raw_yaw = None            # Latest 0..360 BNO055 heading received from ESP32
...
# Turn completion counter - 4 turns/lap x 3 laps, then return to parking
total_turns_completed = 0  # Counter for total completed turns
MAX_TURNS_BEFORE_STOP = 12  # Kept name for display compatibility; now triggers parking

# Parking navigation state
navigation_mode = "WAITING"  # WAITING, EXIT_PARKING, NAVIGATING, FINAL_APPROACH, PARKING, PARKED
parking_phase = "NONE"
parking_side = None           # Outer-wall / parking side: "LEFT" or "RIGHT"
...
# Parking maneuver tuning values. These must be refined during track testing.
PARKING_SPEED = 85
PARKING_REVERSE_SPEED = 75
PARKING_EXIT_ANGLE = 32.0
PARKING_REVERSE_ANGLE = 38.0
PARKING_HEADING_TOLERANCE = 4.0
PARKING_APPROACH_SIDE_DISTANCE = 32.0  # cm from the outer wall while passing the bay
PARKING_INSIDE_SIDE_DISTANCE = 22.0    # cm; side reading expected once inside the bay
...
esp_start_event = threading.Event()  # Set when ESP32 sends "START"
                ...
                if ser.in_waiting > 0:
                    ...
                    if line == "START":
                        ...
                        if len(parts) == 9:
                            ...
                            # DATA packets only begin after the ESP32 button is pressed.
                            # This also recovers if the one-time START line was missed.
...
def complete_turn(count_as_completed=True):
    ...
    # Count only yaw-validated corner completions. Timeouts and cancellations do not count.
    ...
    # After 12 valid corners (three laps), enter the starting/parking section.
...
def calibrate_imu():
    ...
    # Clear any previous unwrapping state before establishing the new zero.
...
def update_parking_controller(current_yaw, front_d, left_d, right_d, parking_blocks):
    ...
    if navigation_mode == "PARKING":
        ...
        if parking_phase == "REVERSE_TOWARD_BAY":
            ...
            # Reverse steering is opposite to the desired heading change.
            steering = SERVO_LEFT if parking_side == "LEFT" else SERVO_RIGHT
...
def decide_steering():
    ...
    # Every physical sensor comes from the same ESP32 DATA packet. Never move
    # using old ultrasonic or old BNO055 information.
    ...
    # Parking actions own both speed and steering. During final approach, coloured
    # traffic signs keep priority until they have been safely passed.
...
def draw_debug(frame, red_blocks, green_blocks, steering, state, parking_blocks=None):
    ...
    # Draw magenta parking-lot limitations independently from traffic signs
...
def main():
    ...
    try:
        ...
        # Test serial connection and keep the robot stopped until ESP32 sends START
        ...
        # The robot starts inside the parking lot. Record its wall side and heading,
        # then use the dedicated exit maneuver before normal lap navigation.
        time.sleep(0.2)
        ...
        while True:
            ...
            # Turn and parking states send their own speed commands inside
            # decide_steering(). Only ordinary obstacle navigation uses BASE_SPEED.
